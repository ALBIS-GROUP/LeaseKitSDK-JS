...
class OrdersService {
	getFilter(params = {}) {
		...
		if (params.search) {
			...
			alternativeSearch.push({ legal_form: new RegExp(params.search, 'i') });
			...
			alternativeSearch.push({ fax: new RegExp(params.search, 'i') });
	...
	getValidDocumentForInsert(data) {
		return db
			...
			.then(items => {
				...
				order.legal_form = parse.getString(data.legal_form).toLowerCase();
				...
				order.fax = parse.getString(data.fax).toLowerCase();
	...
	getValidDocumentForUpdate(id, data) {
		return new Promise((resolve, reject) => {
			...
			if (data.legal_form !== undefined) {
				order.legal_form = parse.getString(data.legal_form);
			}
