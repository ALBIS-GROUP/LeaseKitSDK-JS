...
const addAllProducts = async db => {
	...
	if (productsNotExists) {
		...
		await db.collection('products').insertOne({
			name: 'Racing bike 2018',
			slug: 'racing-bike-2018',
			category_id: catB.insertedId,
			regular_price: 650,
			stock_quantity: 2,
			enabled: true,
			discontinued: false,
		...
		await db.collection('products').insertOne({
			name: 'Racing bike 2019',
			slug: 'racing-bike-2019',
			category_id: catB.insertedId,
			regular_price: 1250,
			stock_quantity: 7,
			enabled: true,
		...
		await db.collection('products').insertOne({
			name: 'Racing bike 2020',
			slug: 'racing-bike-2020',
			category_id: catB.insertedId,
			regular_price: 1800,
			stock_quantity: 4,
			enabled: true,
		...
		await db.collection('products').insertOne({
			name: 'Racing bike 2021',
			slug: 'racing-bike-2021',
			category_id: catB.insertedId,
			regular_price: 2250,
			stock_quantity: 2,
			enabled: true,
	...
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
			...
			<div style="width: 100%; margin-top: 20px;">
			  ...
			  <b>Address 1</b>: {{shipping_address.street}}<br />
...
const addDefaultCheckoutFields = async db => {
	const checkoutFiledsCount = await db
		.collection('checkoutFields')
		.countDocuments({});
	const checkoutFieldsNotExists = checkoutFiledsCount === 0;
	if (checkoutFieldsNotExists) {
		await db.collection('checkoutFields').insertOne({
			name: "fax",
			label: "Fax",
			placeholder: "Fax",
			status: "optional"
		...
		winston.info('- Added default checkout fields');
	}
};
...
const addPaymentMethods = async db => {
	...
	if (paymentMethodsNotExists) {
		await db.collection('paymentMethods').insertOne({
			name: 'Albis Leasing',
			description: "Lease your product through our partner Albis Leasing instead of buying...",
			enabled: true,
			gateway:"albis-leasing",
			conditions: {
				countries: [],
				shipping_method_ids: [],
				subtotal_min: 0,
...
(async () => {
	...
	await addDefaultCheckoutFields(db);
